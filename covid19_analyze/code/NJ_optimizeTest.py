# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 12:51:19 2021
...
import NJ_Polyfit_Optimize as pfo
...
#新泽西数据
df = pd.read_excel("D:/项目/毕业设计/资料/covidtracking/NJ新泽西(全)/新泽西20201101-20210121清洗.xlsx")
#print(df)
...
#    date.append(tempdate[i].strftime('%Y-%m-%d'))
#    
...
#res1[3] = map(int,res1[3])
#
#yy = pd.DataFrame(res1[3])
#
#writer = pd.ExcelWriter('D:/项目/毕业设计/资料/covidtracking/NJ新泽西(全)/1.xlsx')
#
...
writer = pd.ExcelWriter('D:/项目/毕业设计/资料/covidtracking/NJ新泽西(全)/2.xlsx')
...
writer = pd.ExcelWriter('D:/项目/毕业设计/资料/covidtracking/NJ新泽西(全)/3.xlsx')
...
plt.title("New infected cases(ω=0.1)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("New infected cases(ω=0.13)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("New infected cases(ω=0.14)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("New infected cases(ω=0.15)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("Accumulative infected cases(ω=0.1)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("Accumulative infected cases(ω=0.13)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("Accumulative infected cases(ω=0.14)")
plt.show()
...
#plt.xlabel('Date') #设置x轴名称
plt.ylabel('No. of infect person ') #设置y轴名称
plt.title("Accumulative infected cases(ω=0.15)")
plt.show()

#plt.subplot(1,2,2)
#print("新增恢复真实/预测：")
#plt.axes([0.9, 0.1, 0.7, 0.5])
#plt.plot(xdata1, zdata1, 's',label='original values')
#plt.plot(range(0,len(temp5)), temp5, label='pridect')
#plt.title("recoveredIncrease true/predict")
#plt.show()
...
#
## result
##res = scipy.integrate.odeint(Kutta, [S0, P0, E0, I0, R0, D0], t, args=(alpha, beta, gamma, lambd, kappa))
##res = np.array(res)